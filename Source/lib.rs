...
pub use desktop::Fs;
pub use error::Error;
pub use file_path::{FilePath, SafeFilePath};
#[cfg(target_os = "android")]
pub use mobile::Fs;
...
impl OpenOptions {
	...
	/// the same file. This is unlike <code>[seek]\([SeekFrom]::[End]\(0))</
	/// code> followed by `write()`, which has a race between seeking and
	/// writing during which another writer can write, with our `write()`
	...
	/// data accepted a single `write()` call depends on the operating system
	/// and file system. A successful `write()` is allowed to write only part
	/// of the given data, so even if you're careful to provide the whole
	/// message in a single call to `write()`, there is no guarantee that it
	/// will be written out in full. If you rely on the filesystem
	/// accepting the message in a single write, make sure that all data that
	/// belongs together is written in one operation. This can be done by
	/// concatenating strings before passing them to [`write()`].
	///
	...
	/// (using <code>[Seek]::[stream_position]</code>), and restore it before
	/// the next read.
	///
	/// ## Note
	///
	...
	/// guaranteed to be new. If a file exists at the target location, creating
	/// a new file will fail with [`AlreadyExists`] or another error based on
	/// the situation. See [`OpenOptions::open`] for a non-exhaustive list of
	/// likely errors.
	///
...
// implement ScopeObject here instead of in the scope module because it is also
// used on the build script and we don't want to add tauri as a build dependency
impl ScopeObject for scope::Entry {
	...
	fn deserialize<R:Runtime>(
		...
		let path = serde_json::from_value(raw.into()).map(|raw| {
			match raw {
...
pub trait FsExt<R:Runtime> {
	...
	/// Cross platform file system APIs that also support manipulating Android
	/// files.
	fn fs(&self) -> &Fs<R>;
}

impl<R:Runtime, T:Manager<R>> FsExt<R> for T {
	fn fs_scope(&self) -> tauri::fs::Scope { self.state::<Scope>().scope.clone() }
