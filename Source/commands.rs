...
impl From<String> for CommandError {
	fn from(value:String) -> Self { Self::Anyhow(anyhow::anyhow!(value)) }
}
	...
	let entries = entries
		.filter_map(|entry| {
			...
			macro_rules! method_or_false {
				($method:ident) => {
					if let Ok(metadata) = &metadata { metadata.$method() } else { false }
				};
			}
...
pub async fn read<R:Runtime>(
	...
	// This is an optimization to include the number of read bytes (as bigendian
	// bytes) at the end of returned vector so we can use `tauri::ipc::Response`
...
pub async fn read_text_file_lines_next<R:Runtime>(
	...
	let ret = StdLinesResource::with_lock(&lines, |lines| -> CommandResult<Vec<u8>> {
		// This is an optimization to include wether we finished iteration or not (1 or
		// 0) at the end of returned vector so we can use `tauri::ipc::Response`
		// and avoid serialization overhead of separate values.
		match lines.next() {
			Some(Ok(mut bytes)) => {
...
fn default_create_value() -> bool { true }
...
pub async fn size<R:Runtime>(
	...
	if metadata.is_file() {
		...
		let size = get_dir_size(&resolved_path).map_err(|e| {
			format!("failed to get size at path: {} with error: {e}", resolved_path.display())
		})?;
	...
	if let Ok(path) = path {
		...
		scope.forbidden_patterns().iter().any(|p| {
			p.matches_path_with(
				&path,
				glob::MatchOptions {
					// this is needed so `/dir/*` doesn't match files within subdirectories such as
					// `/dir/subdir/file.txt` see: <https://github.com/tauri-apps/tauri/security/advisories/GHSA-6mv3-wm7j-h4w5>
...
impl StdFileResource {
	fn new(file:File) -> Self { Self(Mutex::new(file)) }
...
impl StdLinesResource {
	fn new(lines:BufReader<File>) -> Self { Self(Mutex::new(LinesBytes(lines))) }
...
mod test {
	...
	fn test_lines_bytes() {
		...
		let string2 = BufReader::new(bytes).lines().map_while(Result::ok).collect::<String>();
