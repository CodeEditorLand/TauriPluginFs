...
impl FilePath {
	/// Get a reference to the contained [`Path`] if the variant is
	/// [`FilePath::Path`].
	///
	/// Use [`FilePath::into_path`] to try to convert the [`FilePath::Url`]
	/// variant as well.
	#[inline]
	...
	///
	/// This calls [`Url::to_file_path`](url::Url::to_file_path) if the variant
	/// is [`FilePath::Url`], otherwise returns the contained [PathBuf] as is.
	#[inline]
...
impl SafeFilePath {
	...
	/// [`SafeFilePath::Path`].
	///
	/// Use [`SafeFilePath::into_path`] to try to convert the
	/// [`SafeFilePath::Url`] variant as well.
	#[inline]
	pub fn as_path(&self) -> Option<&Path> {
		match self {
			Self::Url(_) => None,
	...
	///
	/// This calls [`Url::to_file_path`](url::Url::to_file_path) if the variant
	/// is [`SafeFilePath::Url`], otherwise returns the contained [PathBuf] as
	/// is.
	#[inline]
	pub fn into_path(self) -> Result<PathBuf> {
		match self {
	...
	/// Takes the contained [`PathBuf`] if the variant is
	/// [`SafeFilePath::Path`], and when possible, converts Windows UNC paths
	/// to regular paths.
	#[inline]
	pub fn simplified(self) -> Self {
		match self {
...
impl From<&Path> for FilePath {
	fn from(value:&Path) -> Self { Self::Path(value.to_owned()) }
}
...
impl From<&PathBuf> for FilePath {
	fn from(value:&PathBuf) -> Self { Self::Path(value.to_owned()) }
}
...
impl From<url::Url> for FilePath {
	fn from(value:url::Url) -> Self { Self::Url(value) }
}
...
impl TryFrom<FilePath> for PathBuf {
	...
	fn try_from(value:FilePath) -> Result<Self> { value.into_path() }
}

impl TryFrom<SafeFilePath> for PathBuf {
	type Error = Error;

	fn try_from(value:SafeFilePath) -> Result<Self> { value.into_path() }
}
