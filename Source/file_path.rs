...
impl TryFrom<&Path> for SafeFilePath {
	...
	fn try_from(value: &Path) -> Result<Self> {
		SafePathBuf::new(value.to_path_buf()).map(SafeFilePath::Path).map_err(Error::UnsafePathBuf)
	}
}
...
impl TryFrom<&PathBuf> for SafeFilePath {
	type Error = Error;
	fn try_from(value: &PathBuf) -> Result<Self> {
		SafePathBuf::new(value.to_owned()).map(SafeFilePath::Path).map_err(Error::UnsafePathBuf)
	}
}
