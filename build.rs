...
fn main() {
	...
	if !base_dirs.exists() {
		create_dir_all(base_dirs)
			.expect("unable to create autogenerated base directories dir");
	}
...
permissions = [
    ...
    "scope-{lower}-index"
		...
		{
			std::fs::write(permission_path, toml).unwrap_or_else(|e| {
				panic!("unable to autogenerate ${lower}: {e}")
			});
		}
	}
